...
TEMPLATE_TEST_CASE("grit/audio/waveshape: HalfWaveRectifier", "", float, double)
{
    ...
    REQUIRE(shaper.processSample(Float(-2.0)) == Catch::Approx(+0.0));
    REQUIRE(shaper.processSample(Float(-1.0)) == Catch::Approx(+0.0));
    REQUIRE(shaper.processSample(Float(-0.1)) == Catch::Approx(+0.0));
    REQUIRE(shaper.processSample(Float(+0.1)) == Catch::Approx(+0.1));
    REQUIRE(shaper.processSample(Float(+1.0)) == Catch::Approx(+1.0));
    REQUIRE(shaper.processSample(Float(+2.0)) == Catch::Approx(+2.0));
}
